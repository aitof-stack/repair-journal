...
if (window.location.pathname.includes('login.html')) {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (users.repair.length > 0) {
            window.selectedUser = users.repair[0];
            highlightSelectedUser(window.selectedUser.name);
        }
        ...
        document.querySelectorAll('.user-type-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                window.selectedUserType = this.dataset.type;
                window.selectedUser = null;
                
                populateUserList(window.selectedUserType);
                
                if (window.selectedUserType === 'repair') {
...
function login() {
    ...
    if (window.selectedUserType !== 'repair') {
        ...
        if (enteredPassword !== window.selectedUser.password) {
    ...
    const userData = {
        ...
        permissions: getPermissions(window.selectedUserType)
    };
...
// Функция проверки аутентификации (используется в index.html)
...
// Функция выхода (используется в index.html)
function logout() {
